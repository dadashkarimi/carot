...
parser.add_argument('-simplex','--simplex',type=bool,default=False,help="simplex evaluation")
...
#tasks = ["emotion","gambling","wm","motor","lang","social","relational","emotion"]
tasks = ["lang"]
...
def atlas_intrinsic_evaluatin(source,target,task,n_iters,cross_task = None):
    ...
    #os.remove(source+"_"+target+"_frame_size_corr.txt")
...
def simplex_ot(target,train_index,test_index,task,n_iters,alpha,beta):
    print("simplex OT started .. ")
    ...
    for k in range(5):
        ...
        local_atlases = random.sample(local_atlases, k=3)

        G1 =  sinkhorn(local_atlases[0],target,train_index,task,num_time_points)
        G2 =  sinkhorn(local_atlases[1],target,train_index,task,num_time_points)
        G3 =  sinkhorn(local_atlases[2],target,train_index,task,num_time_points)

        test_time_series1 = np.zeros((n,p,num_time_points))
        test_time_series2 = np.zeros((n,p,num_time_points))
        test_time_series3 = np.zeros((n,p,num_time_points))
        ...
        for i in tqdm(test_index,desc="applying OT .."):
            for j in range(num_time_points):
                a1 = all_data[(local_atlases[0],task)][i,:,j]
                a2 = all_data[(local_atlases[1],task)][i,:,j]
                a3 = all_data[(local_atlases[2],task)][i,:,j]

                a1 = normalize(a1) + 1e-6
                a1=(a1)/np.sum(a1)

                a2 = normalize(a2) + 1e-6
                a2=(a2)/np.sum(a2)

                a3 = normalize(a3) + 1e-6
                a3=(a3)/np.sum(a3)

                test_time_series1[i,:,j] = np.transpose(G1[j]).dot(a1)
                test_time_series2[i,:,j] = np.transpose(G2[j]).dot(a2)
                test_time_series3[i,:,j] = np.transpose(G3[j]).dot(a3)

            time_series_orig = all_data[(target,task)][i,:,:]
            time_series_pred = alpha*test_time_series1[i,:,:]+beta*test_time_series2[i,:,:]+(1-alpha-beta)*test_time_series3[i,:,:]
            c1 = generate_correlation_map(time_series_orig,time_series_orig)
            c2 = generate_correlation_map(time_series_pred,time_series_pred)
        ...
        for i in tqdm(range(n_iters/5),desc="Evaluation .."):
            s1, s2 = evaluate(c1_test,c2_test,all_behav[test_index],test_size)
            results[int(k*n_iters/5)+i,:] = (s1,s2,np.mean(results_pearson[(source,target)]),np.mean(results_mse[(source,target)]))
    df = pd.DataFrame(results, columns=['orig','ot','pearson','mse'])
    df.to_csv("simplex_"+target+"_"+task+"_"+str(alpha)+"_"+str(beta)+"_iq.csv")
...
def atlas_ot(source,target,train_index,test_index,task,n_iters,cross_task = None):
    ...
    for k in range(10):
        ...
        if args.save_model:
            print(G[0].shape)
            df = pd.DataFrame(G[0])
            df.to_csv("G_"+source+"_"+target+"_"+task+"_iq.csv")
            #return G, None, None
        ...
        for i in tqdm(test_index,desc="applying OT .."):
            for j in range(num_time_points):
                ...
                a = normalize(a)
                b= np.transpose(G[j]).dot(a)
                b= (b-np.min(a))/(np.max(a)-np.min(a))
                test_time_series[i,:,j] = b
            ...
            print("before")
            if args.save_model:
                time_series_source = all_data[(source,task)][i,:,:]
                c0 = generate_correlation_map(time_series_source,time_series_source)
                c0=c0.reshape((p1,p1))
                df0 = pd.DataFrame(c0)
                df0.to_csv("C0_"+source+"_"+target+"_"+task+"_iq.csv")

                c1=c1.reshape((p2,p2))
                df1 = pd.DataFrame(c1)
                df1.to_csv("C1_"+source+"_"+target+"_"+task+"_iq.csv")

                c2=c2.reshape((p2,p2))
                df2 = pd.DataFrame(c2)
                df2.to_csv("C2_"+source+"_"+target+"_"+task+"_iq.csv")
                return G, None, None
...
if __name__ == "__main__":
    ...
    if args.simplex:
        for task in tasks:
            for alpha in range(0,11,2):
                for beta in range(alpha,11,2):
                    if(not os.path.exists("simplex_"+target+"_"+task+"_"+str(alpha)+"_"+str(beta-alpha)+"_iq.csv")):
                        print(alpha,beta)
                        simplex_ot(target,train_index,test_index,task,100,alpha,beta-alpha)
                    else:
                        print("file exists!")
        sys.exit()
    ...
    if args.intrinsic:
        if task == "all":
            for s in atlases:
                if(not os.path.exists(source+"_"+target+"_frame_size_corr.txt")):
                    atlas_intrinsic_evaluatin(s,target,tasks[0],num_iters)
                else:
                    print("file exists!")
        sys.exit() 
