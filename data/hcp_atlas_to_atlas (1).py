...
def all_way_ot(target,train_index,test_index,task,n_iters,cross_task = None):
    ...
    from collections import defaultdict
    G =  sinkhorn_n(target,train_index,task,num_time_points)

    local_atlases = [x for x in atlases if x != target]
    print(atlases)
    ...
    for i in tqdm(range(n_iters),desc="Evaluation .."):
        s1, s2 = evaluate(c1_test,c2_test,all_behav[test_index],test_size)
        results[i,:] = (s1,s2,mean(results_pearson[(source,target)]),mean(results_mse[(source,target)]))
    ...
    df.to_csv("all_"+target+"_"+task+"_iq.csv")
...
def atlas_ot(source,target,train_index,test_index,task,n_iters,cross_task = None):
    ...
    if n_iters>1:
        ...
        #print("Std IQ Performance of task={0}: orig: {1}, ot: {2} cross_task: {3}".format(task,np.std(results[:,0]),np.std(results[:,1]),cross_task))
        #for source,target in results_pearson:
        #    print('source: {0} target: {1} Task: {2} cross_task: {3} MSE: {4}'.format(source,target,cross_task,task,np.mean(results_mse[(source,target)])))
            #print('source: {0} target: {1} Task: {2} cross_task: {3} Pearson: {4}'.format(source,target,cross_task,task,np.mean(results_pearson[(source,target)])))
...
def sinkhorn_n(target,train_index,task,frame_size):
...
if __name__ == "__main__":
    ...
    if args.source == "all":
        if task == "all":
            for t in atlases:
                for task in tasks:
                    if(not os.path.exists("all_"+t+"_"+task+"_iq.csv")):
