...
get_ot_results <- function(filename,atlas) {
  ...
  inp2 <- data.frame(atlas_data$ot,rep("3-cross",100),rep("task",100))
...
get_data <- function(source,target,task,cross_task) {
  ...
  inp1 <- data.frame(c_data01$value,rep(paste("1-","within"),100),rep(task,100),rep(paste(source,"to",target,sep = " "),100))
  colnames(inp1) <-c("value","method","task","atlas")
  
  inp2 <- data.frame(c_data02$value,rep(paste("1-","within"),100),rep(task,100),rep(paste(target,"to",source,sep = " "),100))
...
my_plot<-function(c_data,pal,col0,col1){
  ...
  p<-grouped_ggbetweenstats(
    ...
    ggplot.component = list(ggplot2::scale_color_manual(values = c(col0,col1,col1)),
                            ...
                            ggplot2::scale_y_continuous(limits = c(-0.25, 0.9)
...
p1<-my_plot(c_data1,"default_jama","gold","blue")
p2<-my_plot(c_data2,"default_jama","gold","blue")   

#jpeg(paste(source,target,task1,task2,".jpg",sep="_"),width = 90)
#pdf(paste(source,target,task1,task2,".pdf",sep="_"),width = 10)
Cairo::Cairo(45,30,
             file = paste(paste(source,target,task1,task2,"cross",sep="_"), ".png", sep = ""),
...
ggstatsplot::combine_plots(
  ...
  annotation.args = list(
    title = paste("Between Atlas - Cross Task Optimal Transport: ",toupper(task1)," (TOP), ",toupper(task2)," (Bottom)"))
)
