...
my_plot<-function(cdata,task,target,id){
  ...
  p <- ggplot(cdata, aes(x=sample, y=value)) + 
    geom_bar(stat='identity',fill = wes_palette("Rushmore1")[1:5],colour="black")+
    geom_errorbar( aes(x=sample, ymin=value-std, ymax=value+std), width=0.4, colour="black",size=.3)+
    theme_minimal(base_size = 20)+
    scale_y_continuous(limits=c(0,0.7))+
    labs(title="Leave k-atlas out",subtitle = paste(dict[[target]],"-",task), x="k", y = "Pearson")
  return(p)
}
...
tasks <-c("relational","social")
targets <- c("brainnetome","schaefer","shen")

Cairo::Cairo(28,20,
             file = paste("atlas_sample.png"),
...
for(i in 1:2){
  for(target in targets){
    cdata<- get_data(tasks[i],target)
    p<-my_plot(cdata,tasks[i],target,1)
    my_plots[[p_index]]<-p
    p_index <- p_index+1
  }
}
...
ggarrange(plotlist = my_plots,nrow=2,ncol=2)
